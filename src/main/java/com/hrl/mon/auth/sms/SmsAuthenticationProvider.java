...
//        PhoneDTO phoneDTO =
//                Optional.ofNullable(
//                        (PhoneDTO)redisTemplate.opsForValue().get(RedisKeyPrefix.SMS_CODE_PREFIX+phoneNumber))
//                        .orElseThrow(() -> new ApiException(CommonExceptionEnum.SMS_VERIFY_CODE_INVALID));
//        //是否已验证
//        ApiAssert.isFalse(phoneDTO.getVerify(),CommonExceptionEnum.SMS_VERIFY_CODE_INVALID);
//
//        //是否有效期内
//        LocalDateTime lastSendTime = phoneDTO.getLastSendTime();
...
//
//        //校验验证码
//        Optional.of(phoneDTO).filter(x -> x.getCode().equals(code))
//                .orElseThrow(() -> new ApiException(CommonExceptionEnum.SMS_VERIFY_CODE_ERROR));
//
//        phoneDTO.setVerify(true);
//        LocalDateTime midnight = LocalDateTime.now().plusDays(1).withHour(0).withMinute(0).withSecond(0).withNano(0);
//        long millSeconds = ChronoUnit.MILLIS.between(LocalDateTime.now(),midnight);
//        redisTemplate.opsForValue().set(RedisKeyPrefix.SMS_CODE_PREFIX + phoneNumber, phoneDTO, millSeconds, TimeUnit.MILLISECONDS);
